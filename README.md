...
- **Firm's dashboard:** <https://dntywntme.github.io/2026-08-15-SF-0HumanCompanyHack-firm/>
...
Both sides publish independently. The client asserts it paid; the firm reports
receiving it; the two pages agree without sharing a database. Every figure lives
on the firm's ledger, not in this file.
...
metadata. The boundary itself is enforced from the firm's side by credential
...
inside the firm. An attack the firm survives is only evidence if the attacker was
never inside.
...
duplicated from the firm's page.
